use super::exp::{format_int_exp, format_uint_exp};
...
pub fn parse_tokens<'a, I: IntoIterator<Item = (&'a DynSolType, &'a str)>>(
    ...
    params
        .into_iter()
        .map(|(param, value)| DynSolType::coerce_str(param, value))
        .collect()
}
...
) -> Result<Value, color_eyre::Report> {
...
impl StructDefinitions {
    ...
    pub fn get(&self, key: &str) -> Result<Option<&[(String, String)]>, color_eyre::Report> {
